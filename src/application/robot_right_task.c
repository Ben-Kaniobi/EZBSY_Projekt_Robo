...
*
* Procedures : 	vRobotRightTask( void *pvParameters )
*
* \author plats1
*
* \version 1.0.0
...
/******************************************************************************/
/*! \brief Function that controls the right robot arm
*
* Function : This function controls the right robot arm by sending appropriate CAN
...
void vRobotRightTask( void *pvParameters )
{
	...
	createCANMessage( CAN_ID_RIGHT_ROBOT, CAN_INSTRUCTION_DLC, payloadRight[ WaitingPosition ] );
	...
	xSemaphoreGive( xSemaphoreRightConveyor );

	/* Prepare robot to take up an ECTS */
	createCANMessage( CAN_ID_RIGHT_ROBOT, CAN_INSTRUCTION_DLC, payloadRight[ PrepareToTakeEcts ] );
...
//        }
    	...
    	createCANMessage( CAN_ID_RIGHT_ROBOT, CAN_INSTRUCTION_DLC, xEctsGrabPosition );
        vTaskDelay( 500 / portTICK_RATE_MS );
    	...
    	createCANMessage( CAN_ID_RIGHT_ROBOT, CAN_INSTRUCTION_DLC, payloadRight[ LiftEctsUp ] );
    	...
    	createCANMessage( CAN_ID_RIGHT_ROBOT, CAN_INSTRUCTION_DLC, payloadRight[ RotateToUnloadTray ] );
        vTaskDelay( 1500 / portTICK_RATE_MS );
    	...
    	xSemaphoreTake( xSemaphoreEctsTray, portMAX_DELAY );
    	{
			...
			createCANMessage( CAN_ID_RIGHT_ROBOT, CAN_INSTRUCTION_DLC, payloadRight[ PrepareToUnloadEcts ] );
		    vTaskDelay( 1200 / portTICK_RATE_MS );


			/* Unload ECTS */
			createCANMessage( CAN_ID_RIGHT_ROBOT, CAN_INSTRUCTION_DLC, payloadRight[ UnloadEctsToTray1 ] );
			...
			createCANMessage( CAN_ID_RIGHT_ROBOT, CAN_INSTRUCTION_DLC, payloadRight[ UnloadEctsToTray2 ] );
			vTaskDelay( 600 / portTICK_RATE_MS );

			/* Remove robot arm from the drop off tray by rotating back to the conveyor */
			createCANMessage( CAN_ID_RIGHT_ROBOT, CAN_INSTRUCTION_DLC, payloadRight[ RotateToConveyor ] );
		    vTaskDelay( 1200 / portTICK_RATE_MS );
